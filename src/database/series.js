...
async function getExistingSeriesIDByName(client, seriesName, isGraphicNovelSeries) {
  ...
  if (
    ...
    result.rows[0].is_graphic_novel_series !== isGraphicNovelSeries
  )
    throw new Error(
      `! Graphic novel series mismatch, this needs to be manually reviewed`
    )
  return result.rows.length === 1 ? result.rows[0].series_id : null
}

async function insertNewSeriesAndGetID(
  client,
  seriesName,
  publisherID,
...
async function commitSeries(client, seriesName, publisherID, isGraphicNovelSeries) {
  ...
  const seriesID = await getExistingSeriesIDByName(
    client,
    seriesName,
    isGraphicNovelSeries
  )
