...
async function getScrapedSeriesName(seriesLink) {
  ...
  if (!seriesNameHTML)
    logger.warn(
      `! Failed request to ${seriesLink} and could not retrieve series name`
    )
...
async function getScrapedRelease(releaseLink, releaseFormat) {
  ...
  const seriesName = seriesLink
    ? `${await getScrapedSeriesName(baseURL + seriesLink)}`
    : ""
  ...
  const scrapedRelease = {
    ...
    coverURL:
      baseURL +
      $(".mainContentImage .ImageContainer", newReleaseResponse).attr("href"),
    description: $("div.Text", newReleaseResponse)
      ...
      .replace(/\s+/g, " ")
      .trim(),
    creators: $(".Creators", newReleaseResponse)
      .text()
      .replace(/\s+/g, " ")
      .trim()
      .split(" "),
...
function filterOutReleasesWithFlaggedPublishers(releases) {
  ...
  const filteredReleases = releases.filter(
    (release) =>
      flaggedPublishers.includes(release.publisher.name.toUpperCase()) === false
  )
...
async function getScrapedPreviewsWorldReleases() {
  ...
  for (const [index, { link, format }] of scrapedLinksAndFormats.entries()) {
    logger.info(
      `# Started scraping release ${index + 1} of ${scrapedLinksAndFormats.length}`
    )
    scrapedReleases.push(await getScrapedRelease(link, format))
    logger.info(
      `# Finished scraping release ${index + 1} of ${
        scrapedLinksAndFormats.length
      }\n`
    )
  }
  logger.info(`### Finished scraping releases\n`)

  const filteredScrapedReleases = filterOutReleasesWithFlaggedPublishers(
    scrapedReleases
  )
  const releases = []
  ...
  filteredScrapedReleases.forEach((release, index) => {
    try {
      logger.info(
        `# Parsing release ${index + 1} of ${filteredScrapedReleases.length}`
      )
      ...
      logger.info(
        `# Finished parsing release ${index + 1} of ${
          filteredScrapedReleases.length
        } \n`
      )
    } catch (error) {
      logger.error(
